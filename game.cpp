...
void Game::play() 
{
    ...
    std::thread searchForIntersectsThread(&Game::searchForIntersects, this);
    ...
    keyboardThread.join();
    searchForIntersectsThread.join();
}
...
void Game::spawnPlate() 
{
    ...
    int firePower = std::rand() % 10 + 25;
    Velocity velocity;
    velocity.x = std::cos(DEG_TO_RAD(ANGLE)) * firePower * 1.5; 
...
{
    constexpr int firePower = 20; 
    Velocity velocity;
    velocity.x = firePower;
    velocity.y = firePower * 0.5;
